# auto-ingest Specification

## Purpose

Tier 1. Let a user drop files into `notes/` and have Claude pick them up on demand,
without re-pasting them into chat. An invocation-triggered sweep ingests new-or-changed
files, tracks processed state in an external ledger (`.wiki-ingest-ledger.json`) so the
user's own files are never edited, and re-ingests changed files idempotently — never
duplicating pages. This is not a background watcher; a continuously-watching inbox is a
deferred Tier 2 concern (see `onboarding/spec.md`). The operational procedure lives in the
wiki's `CLAUDE.md` (defaults in `defaults/CLAUDE.md`).
